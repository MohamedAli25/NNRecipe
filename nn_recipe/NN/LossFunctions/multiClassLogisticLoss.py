...
class MClassLogisticLoss(LossFunction):
    def _compute_loss(self, Y, Y_hat):
        ...
        #     print(Y_hat[batch].shape)
        #     expZ = np.exp(Y_hat[batch] - np.max(Y_hat[batch])) # softmax here is a row vector for each example
        #     self.__softmax_value = expZ / np.sum(expZ, axis=1).reshape(-1, 1)
        #     print(-1*np.log(self.__softmax_value[0, np.argmax(Y.T, axis=1)].reshape(-1,1)))
        #     losses[batch] = -1*np.log(self.__softmax_value[0, np.argmax(Y.T, axis=1)].reshape(-1,1))
        # return losses
